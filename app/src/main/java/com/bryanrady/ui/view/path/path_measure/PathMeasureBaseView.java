...
public class PathMeasureBaseView extends View {

    private static final String TAG = "PathMeasureBaseView";
    ...
    public PathMeasureBaseView(Context context) {
        super(context);
        init();
    }
    ...
    private void testNextContour(Canvas canvas) {
        ...
        path.op(path1,path2, Path.Op.XOR);  //XOR得到的效果是交集的补集
    ...
    private void testGetSegment(Canvas canvas) {
        ...
        canvas.drawTextOnPath("顺时针",path,0,0,mPaint);

        Path dst = new Path();
        // 截取一部分存入dst中，并使用 moveTo 保持截取得到的 Path 第一个点的位置不变
        boolean segment = measure.getSegment(200, 600, dst, false);
        Log.e(TAG, "segment = "+ segment);

        /**
         * 如果 startD、stopD 的数值不在取值范围 [0, getLength] 内，或者 startD == stopD 则返回值为 false，不会改变 dst 内容。
         * 如果在安卓4.4或者之前的版本，在默认开启硬件加速的情况下，更改 dst 的内容后可能绘制会出现问题，请关闭硬件加速或者给
         * dst 添加一个单个操作，例如: dst.rLineTo(0, 0) 可以用以下规则来判断 startWithMoveTo 的取值
         */
