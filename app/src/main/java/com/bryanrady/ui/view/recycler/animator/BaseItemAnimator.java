...
/**
 * 把DefaultItemAnimator中关于动画的初始、开始、结束状态的动画和属性设置处抽取了抽象方法而已，
 * 其它的动画等待、执行、移除、还原、让位移动逻辑都是适用任何动画的
 *
 * 这里需要注意的地方是不能为动画设置自己想要的动画时间，要调用getChangeDuration()，getAddDuration()，getChangeDuration()来设置
 * 动画时间或者延时，这样才能够保持应用RecyclerView.ItemAnimator动画时间的一致性。所以当设置的时间后，为了保持动画一致性，
 * 还是会在BaseItemAnimator重新设置为对应的RecyclerView.ItemAnimator动画时间。
 */
public abstract class BaseItemAnimator extends SimpleItemAnimator {
    ...
    public void animateMoveImpl(final ViewHolder holder, int fromX, int fromY, int toX, int toY) {
        final View view = holder.itemView;
...
//    网上大概两种方案：
//
//    1. app下的build.gradle添加如下配置 24.1.0具体看你的版本信息,另外有篇帖子
